...
在这里开始写你的正文内容...Scoop 是一款 Windows 平台的上的软件包管理器，可以让你像在 Linux 上一样简单、方便地安装软件，并自动配置环境。软件及其配置文件都在 scoop 文件夹下，无论是重装电脑还是换一台设备，只要把 scoop 文件夹移过去，在目录下运行：
